...
def branch(point, angle, length, delta, step):
    if length < 1:
        return
    v1 = sd.get_vector(start_point=point, angle=angle, length=length, width=3)
    v1.draw()
    ...
    next_length = length * step
    branch(point=next_point, angle=next_angle, length=next_length, delta=delta, step=step)


for delta in range(0, 51, 10):
    branch(point=point_0, angle=90, length=150, delta=delta, step=0.5)
